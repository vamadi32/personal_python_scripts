...
def delete_user(token, user_id):
        ...
        if response.status_code in [200, 204]:
            ...
            print(f"Error deleting user ID {user_id}: HTTP Status Code {response.status_code}")
            return False
...
def main():
    ...
    users_without_devices = [
        user_details['user']['id'] for user_details in user_details_list 
    ...
    # Limit to deleting up to 10 users
    users_to_delete = users_without_devices[:10]
    #print(users_to_delete)

    print("Deleting up to 10 users without devices:")
